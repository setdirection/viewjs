(function() {
      ...
      render: function(filename) {
        ...
        if (typeof filename === 'string') {
          callback = function(context) {
            ...
            process_view_nodes(output, this);
            return output;
          };
        } else {
          callback = filename;
  ...
  process_view_nodes = function(output, context) {
    var attribute, div, element, method_name, node, nodes, params, response, template_node, template_nodes, _i, _j, _len, _len2, _results;
    template_nodes = [];
    if (output.length != null) {
      for (_i = 0, _len = output.length; _i < _len; _i++) {
        template_node = output[_i];
        if (template_node.getElementsByTagName != null) {
          template_nodes = template_nodes.concat(template_node.getElementsByTagName('*'));
        }
      }
    } else if (output.getElementsByTagName != null) {
      template_nodes = output.getElementsByTagName('*');
    }
    _results = [];
    for (_j = 0, _len2 = template_nodes.length; _j < _len2; _j++) {
      element = template_nodes[_j];
      _results.push((function() {
        var _i, _j, _len, _len2, _ref, _results;
        if ((element.tagName != null) && element.tagName.toLowerCase().match(/^view\:/)) {
          params = {};
          ...
          method_name = element.tagName.toLowerCase().replace(/^view\:/, '');
          div = this.document.createElement('div');
          response = context[method_name](params);
          if ((response != null) && (response.nodeType != null)) {
            nodes = [response];
          } else {
          ...
          for (_j = 0, _len2 = nodes.length; _j < _len2; _j++) {
            node = nodes[_j];
            _results.push(element.parentNode.insertBefore(node, element));
          }
          return _results;
        }
      }).call(this));
