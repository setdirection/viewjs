(function() {
  ...
  ViewServer.extend({
    compileTemplates: function(as_string, document) {
      ...
      suffix = ')(___obj))})';
      files_with_extension(this.templates, /\.html$/).map(__bind(function(filename) {
        var content;
      ...
      files_with_extension(this.templates, /\.jade$/).map(__bind(function(filename) {
        var content, output;
        content = String(fs.readFileSync(filename));
      ...
      files_with_extension(this.templates, /\.eco$/).map(__bind(function(filename) {
        var content, output;
        content = String(fs.readFileSync(filename));
          ...
          return __bind(function(script) {
            ...
            tag.onload = __bind(function() {
              if (script.match(/(^|\/)view\.js$/)) {
                window.View.extend({
                  routes: this.routes,
                  cache: this.compileTemplates(false, window.document)
                });
              }
