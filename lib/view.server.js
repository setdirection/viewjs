(function() {
  var ViewServer, ViewServerManager, XMLHttpRequest, array_flatten, array_from, bind_extend_handler, create_empty_document, create_server, deffered, eco, environments, express, extend, files_with_extension, fs, http, is_array, is_directory, jade, jsdom, proxy, url, _;
  ...
  ViewServer.extend({
    respond: function(request, response) {
      ...
      return this.createWindow(request, __bind(function(window) {
        if (window.View != null) {
          window.View.extend({
            on: {
              route: __bind(function(view_instance) {
                var output;
                output = this.renderWindow(window, request);
          ...
          return response.send(this.renderWindow(window, request));
        }
      }, this));
    },
    createWindow: function(request, callback) {
      ...
      this.trigger('window:created', window);
    ...
    renderWindow: function(window, request) {
      var javascripts, output, stylesheets;
      this.trigger('window:render', window, request);
      output = window.document.documentElement.innerHTML;
      stylesheets = (request._stylesheets || []).map(__bind(function(href) {
  ...
  ViewServer.extend({
    proxy: function(options) {
      ...
      if (!options.base.match(/\/$/) && !options.url) {
        options.base = options.base + '/';
      }
      base = options.url ? options.base : new RegExp('^' + options.base.replace(/\//g, '\\/') + '(.+)$');
      ...
      return this.server.get(base, function(request, response) {
        ...
        request_options = {
          ...
          path: options.url ? url_components.pathname : '/' + request.params[0],
