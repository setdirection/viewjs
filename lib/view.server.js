(function() {
  var ViewServer, ViewServerManager, array_flatten, array_from, bind_extend_handler, create_server, deffered, eco, environments, express, extend, files_with_extension, fs, http, is_array, is_directory, jade, process_script_arguments, proxy, setup_static_provider_and_routes, spawn, url, _;
  ...
  ViewServer.extend({
    compileTemplates: function(as_string, document, templates_dir) {
      ...
      compiled = {};
      child_nodes_from_html = "(function(html){    var doc = (typeof(document) != 'undefined' ? document : View.document);    var div = doc.createElement('div');    if(!html.match(/^\\</)){      div.innerHTML = html;      return div;    }else{      div.innerHTML = html;      return div.childNodes.length === 1 ? div.childNodes[0] : div.childNodes;    }  })";
  ...
  ViewServer.extend({
    stylesheets: function(stylesheets) {
      ...
      for (_i = 0, _len = styles.length; _i < _len; _i++) {
        style = styles[_i];
        _results.push(style.match(/^https?\:/) ? add_style(style) : is_directory(style) ? files_with_extension(style, /\.css$/).map(add_style) : add_style(style));
      }
      return _results;
    }
  });
  ...
  ViewServer.extend({
    javascripts: function() {
      return process_script_arguments.call(this, arguments, '_javascripts');
    }
  });
  ViewServer.extend({
    extend: {
  ...
  ViewServer.extend({
    execute: function() {
      return process_script_arguments.call(this(arguments, '_execute'));
    }
  });
  ViewServer.extend({
    extend: {
  ...
  process_script_arguments = function(args, target) {
    var add_script, script, _i, _len, _ref, _results;
    this[target] || (this[target] = []);
    add_script = __bind(function(script) {
      if (!(__indexOf.call(this[target], script) >= 0)) {
        return this[target].push(script);
      }
    }, this);
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      script = _ref[_i];
      _results.push(script.match(/^https?\:/) ? add_script(script) : is_directory(script) ? files_with_extension(script, /\.js$/).map(add_script) : add_script(script));
    }
    return _results;
  };
  ViewServer.extend({
  ...
  ViewServer.extend({
    respond: function(request, response) {
      var args, command, envs, execute, javascripts, json_args, stylesheets, _i, _len, _ref;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (args.execute) {
          execute = request._execute.concat(args.execute);
        }
      }
  ...
  ViewServer.extend({
    proxy: function(options) {
      ...
      return this.server.get(base, function(request, response) {
        var key, proxy_request, request_options, value, _ref, _ref2;
        request_options = {
        ...
        if (process.env.HTTP_PROXY != null) {
          ...
          _ref = /[http[s]?:\/\/]?([^:]+):(\d+)\/?/.exec(process.env.HTTP_PROXY), request_options.host = _ref[0], request_options.port = _ref[1];
        }
        if (options.url) {
