(function() {
  var RouteResolver, View, ViewServer, ViewServerManager, array_flatten, array_from, bind_extend_handler, cache_filename_from_url, create_server, deffered, eco, environments, express, extend, file_exists, files_with_extension, fs, history_available, http, is_array, is_directory, jade, process_asset_arguments, proxy, setup_static_provider_and_routes, url, view_name_from_request, _, _ref;
  ...
  url = require('url');
  _ref = require(__dirname + '/view.client.js'), RouteResolver = _ref.RouteResolver, View = _ref.View;
  ...
  file_exists = function(path, callback) {
    if (callback) {
  ...
  cache_filename_from_url = function(url) {
    var filename;
    filename = this.public.replace(/\/$/, '') + '/../cache' + url;
    if (url === '' || url.match(/\/$/)) {
      filename += 'index.html';
  ...
  ViewServer.extend({
    cache: function(url, contents, callback) {
      var directory, filename;
      if (!this.public) {
        this.trigger('error', 'public directory is not defined');
      }
      filename = cache_filename_from_url.call(this, url);
      directory = require('path').dirname(filename);
      if (!file_exists(directory)) {
        return require(__dirname + '/mkdirs').mkdirs(directory, 0777, function(err) {
  ...
  ViewServer.extend({
    cacheExists: function(url, callback) {
      var filename;
      filename = cache_filename_from_url.call(this, url);
      return file_exists(filename, callback);
    }
  });
  ViewServer.extend({
    sendCache: function(url, response) {
      var filename;
      filename = cache_filename_from_url.call(this, url);
  ...
  view_name_from_request = function(request) {
    var key, params, view_name;
  ...
  ViewServer.extend({
    respond: function(request, response) {
      ...
      return this.cacheExists(request.originalUrl, __bind(function(cache_available) {
        var args, command, defaultPort, envs, execute, hostHeader, item, javascripts, json_args, location, meta, protocol, stylesheets, _i, _j, _len, _len2, _meta, _ref, _ref2, _ref3;
        if (false) {
          return this.sendCache(request.originalUrl, response);
        } else {
          ...
          meta = _.uniq(_meta);
          if (!this.location) {
            protocol = (_ref2 = this.protocol) != null ? _ref2 : 'http:';
            defaultPort = (_ref3 = this.defaultPort) != null ? _ref3 : protocol === 'https:' ? 443 : 80;
            hostHeader = request.header('Host');
            location = {
              protocol: protocol,
              host: hostHeader,
              hostname: hostHeader.replace(/:.*/, ''),
              port: ~hostHeader.indexOf(':') ? parseInt(hostHeader.replace(/.*:/, '')) : defaultPort
            };
          } else {
            location = _.clone(this.location);
          }
          ...
          console.log(command);
          console.log('');
          return require('child_process').exec(command, __bind(function(error, stdout, stderr) {
            var send_response;
            if (error || ((stderr != null) && stderr !== '')) {
              ...
              if (this.cache[view_name] != null) {
                return this.cache(request.originalUrl, stdout, send_response);
              } else {
    ...
    proxy: function(options) {
      ...
      return this.server.get(base, function(request, response) {
        ...
        request_options = {
          ...
          path: option.base + request.params[0],
