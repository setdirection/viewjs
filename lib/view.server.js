(function() {
  var ViewServer, ViewServerManager, XMLHttpRequest, array_flatten, array_from, bind_extend_handler, create_server, deffered, eco, environments, express, files_with_extension, fs, http, is_array, is_directory, jade, jsdom, proxy, _;
  ...
  ViewServer.extend({
    extend: {
      public: function(public_dir) {
        ...
        return this.server.use(express.staticProvider(this.public));
      }
    }
  });
  ...
  ViewServer.extend({
    compileTemplates: function(as_string, document) {
      ...
      suffix = ')(___obj))})';
      files_with_extension(this.templates, /\.html$/).map(function(filename) {
        var content;
        content = String(fs.readFileSync(filename));
        content = content.replace(/\\/g, '\\\\').replace(/\'/g, '\\\'');
        return compiled[filename.substring(this.templates.length + 1)] = "(function(){return " + child_nodes_from_html + "('" + content + "')})";
      });
      files_with_extension(this.templates, /\.jade$/).map(function(filename) {
        var content, output;
        content = String(fs.readFileSync(filename));
        output = require('jade').compile(content).toString();
        output = prefix + output + suffix;
        return compiled[filename.substring(this.templates.length + 1)] = output;
      });
      files_with_extension(this.templates, /\.eco$/).map(function(filename) {
        var content, output;
        content = String(fs.readFileSync(filename));
  ...
  ViewServer.extend({
    extend: {
      routes: function(routes) {
        ...
        for (path in routes) {
          view = routes[path];
          _results.push(typeof view === 'function' ? this.server.get(path, view) : (this.routes[path] = view, this.server.get(path, __bind(function(request, response) {
  ...
  ViewServer.extend({
    respond: function(request, response) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (args.stylesheets) {
          request._stylesheets = request.stylesheets.concat(args.stylesheets);
        }
        if (args.javascripts) {
          request._javascripts = request.javascripts.concat(args.javascripts);
        }
        if (args.execute) {
          request._execute = request.javascripts.concat(args.execute);
        }
      }
      _.uniq(request._stylesheets);
      _.uniq(request._javascripts);
      _.uniq(request._execute);
      return this.createWindow(request, function(window) {
        if (window.View != null) {
          window.View.extend({
            on: {
              route: function(view_instance) {
                return response.send(this.renderWindow(window));
              }
            }
          });
    ...
    createWindow: function(request, callback) {
      ...
      request._stylesheets().map(function(href) {
        var tag;
      ...
      request._javascripts().map(function(src) {
        var tag;
          ...
          return __bind(function(script) {
            ...
            tag.onload = __bind(function() {
              if (script.match(/view\.js$/)) {
                window.View.extend({
                  routes: this.routes,
                  cache: this.compileTemplates(false, document)
                });
              }
    ...
    renderWindow: function(window) {
      var output, script_fragment, script_regexp, style_fragment, style_regexp;
      ...
      script_regexp = new RegExp(script_fragment + this.public, 'g');
      style_regexp = new RegExp(style_fragment + this.public, 'g');
      output = output.replace(script_regexp, script_fragment + '/');
      output = output.replace(style_regexp, style_fragment + '/');
      output = output.replace(/\/view.js"\>\<\/script\>/, "/view.js\"></script><script type=\"text/javascript\">      View.extend({        routes: " + (this.JSONFromRoutes()) + ",        cache: " + (this.compileTemplates(true)) + "      });    </script>    ");
      return "<!DOCTYPE html>\n<html>\n  " + output + "\n</html>";
    }
  });
  ViewServer.extend({
  ...
  ViewServer.extend({
    env: function(request, envs) {
      var callback, env_name, should_call, _results, _results2;
      if (typeof request === 'object' && (request.set != null)) {
        ...
        for (env_name in envs) {
          ...
          _results2.push(should_call ? (callback.call(this, request), true) : false);
        }
        return _results2;
      }
    }
