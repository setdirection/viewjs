(function() {
  var Builder, Logger, RouteResolver, Router, View, ViewManager, array_flatten, array_from, array_without_value, attribute_map, attribute_translations, bind_extend_handler, cache, create_router, deffered, delegate_events, dispatcher, element_cache, environments, extend, has_change_callback, ie, ie_attribute_translation_sniffing_cache, ie_attribute_translations, initial_route, is_$, is_array, is_collection, is_element, is_model, is_view, named_param, ordered_routes, params_from_ordered_params_and_route, process_node_argument, proxy, router_initialized, routes_by_path, routes_by_view, routes_regexps_by_path, set_element, splat_param, supported_events, supported_html_tags, tag, tag_name, wrap_function, _fn, _i, _len;
  ...
  is_element = function(element) {
    return Boolean((element != null ? element.nodeType : void 0) === 1 || (element != null ? element.nodeType : void 0) === 2);
  };
  is_$ = function($) {
    var _ref;
  ...
  View.extend({
    env: function(envs) {
      ...
      if (arguments.length === 2) {
        ...
        should_call = typeof environments[env_name] === 'boolean' ? environments[env_name] : environments[env_name]();
        if (should_call) {
  ...
  View.extend({
    element: function() {
      ...
      if (this.name) {
        element.setAttribute('data-view', this.name);
      }
    ...
    delegate: function(events) {
      this._delegatedEvents = events;
      return delegate_events.call(this, events);
    }
  });
  View.extend({
    extend: {
      ...
      $: function(dom_library, discard) {
        if (((typeof jQuery != "undefined" && jQuery !== null) && dom_library === jQuery) || ((typeof Zepto != "undefined" && Zepto !== null) && dom_library === Zepto)) {
          this._$ = dom_library;
        } else {
          this.trigger('error', 'Unsupported DOM library specified, use jQuery or Zepto', dom_library);
        }
        discard();
        return View.extend({
          before: {
            tag: function() {
              var event, next, _i, _len, _ref, _results;
              _ref = arguments, next = _ref[0], arguments = 2 <= _ref.length ? __slice.call(_ref, 1) : [];
              this._$(next.apply(this, arguments));
              _results = [];
              for (_i = 0, _len = supported_events.length; _i < _len; _i++) {
                event = supported_events[_i];
                _results.push($[event] = wrap_function($[event], __bind(function() {
                  var _ref;
                  _ref = arguments, next = _ref[0], arguments = 2 <= _ref.length ? __slice.call(_ref, 1) : [];
                  return arguments[arguments.length - 1] = proxy(arguments[arguments.length - 1], this);
                }, this)));
              }
              return _results;
            }
  ...
  set_element = function(element) {
    ...
    this[0] = element;
    if (this._$) {
      return extend(this.$, this._$(this[0]));
    }
  };
  delegate_events = function(events, element) {
    var event_name, key, method_name, process_item, selector, _ref, _ref2, _results;
    ...
    for (key in events) {
      ...
      _results.push((function() {
        ...
        if (typeof key === 'string') {
          _ref = key.match(/^(\w+)\s*(.*)$/), event_name = _ref[0], selector = _ref[1];
          return process_item.call(this, event_name, selector, method_name);
        } else {
          _results = [];
          for (selector in key) {
            method_name = key[selector];
            _results.push(process_item.call(this, event_name, selector, method_name));
          }
          return _results;
        }
      }).call(this));
  ...
  View.extend({
    ...
    render: function(options) {
      ...
      if (element) {
        if (!is_element(element) && !is_array(element)) {
  ...
  RouteResolver = function() {
    var fragment, ordered_params, param_matcher, param_name, params, path, response, route, router_params, url, view, _i, _len, _ref;
    ...
    if (typeof arguments[0] === 'object') {
      ...
      for (_i = 0, _len = ordered_routes.length; _i < _len; _i++) {
        route = ordered_routes[_i];
        path = route[0], view = route[1];
        ViewManager(view);
  ...
  View.extend({
    extend: {
      routes: function(routes, discard) {
        var dependent_views, path, regexp, route, view, _i, _len;
        dependent_views = [];
        ...
        Router.mixin.push([
          'initialize', function(next) {
            ...
            this.on({
              ready: function() {
                RouteResolver(initial_route, function() {});
                return View.env({
                  browser: function() {
  ...
  dispatcher = function(view_instance, params, callback) {
    var did_change, did_change_observer, next;
    did_change = false;
    ...
    next = function() {
      ...
      hide = function() {
        ...
        console.log(view_instance[0]);
        _ref = view_instance[0].parentNode.childNodes;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          sibling = _ref[_i];
      ...
      remove = function() {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          sibling = _ref[_i];
          _results.push(sibling !== view_instance[0] ? view_instance[0].parentNode.removeChild(sibling) : void 0);
        }
        return _results;
      };
      View.env({
